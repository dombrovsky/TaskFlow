namespace TaskFlow.Tests
{
    ...
    [TestFixture]
    public abstract class TaskSchedulerBaseFixture<T>
        where T : ITaskScheduler
    {
        ...
        {
            var counter = 0;
            var tasks = Enumerable.Range(0, 10).Select(i => _sut.Enqueue(() => Interlocked.Increment(ref counter) == i + 1).AsTask()).ToArray();
        ...
        public void Enqueue_ReturnedTaskShouldBeFailedWhenTaskFuncFailed()
        {
            ...
            Assert.That(async () => await task.ConfigureAwait(false), Throws.InvalidOperationException.With.Message.EqualTo("Failure"));
